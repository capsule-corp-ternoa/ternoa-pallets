...
//! ChainId - The identifier for this chain (a constant). This must be unique and must not collide
//! with existing IDs within a set of bridged chains. PalletId - Identifier of the pallet (a
//! constant). ProposalLifetime -
//! RelayerVoteThreshold - Initial number of votes required for a proposal to be executed (see
//! [RelayerVoteThreshold] in storage section). WeightInfo- Weight information for extrinsics in
//! this pallet.
...
//! FungibleTransfer(ChainId, DepositNonce, ResourceId, U256, Vec<u8>) - A fungible asset was
//! relayed. VoteFor(ChainId, DepositNonce, T::AccountId) - Vote submitted in favour of proposal
...
impl<T: Config> Pallet<T> {
	...
	/// Commits a vote against the proposal and cancels it if more than (get_relayers.len() -
