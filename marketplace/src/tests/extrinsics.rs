...
use crate::{
	tests::mock, CompoundFee, Error, Event as MarketplaceEvent, MarketplaceData, MarketplaceId, MarketplaceType, Sale,
};
...
mod create_marketplace {
	...
	fn create_marketplace() {
		ExtBuilder::new_build(vec![(ALICE, 1000)]).execute_with(|| {
			...
			assert_eq!(Balances::free_balance(ALICE), alice_balance - Marketplace::marketplace_mint_fee());

			// Events checks.
			let event = MarketplaceEvent::MarketplaceCreated { marketplace_id, owner: data.owner, kind: data.kind };
...
mod set_marketplace_owner {
	...
	fn set_marketplace_owner() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			// Events checks.
			let event = MarketplaceEvent::MarketplaceOwnerSet { marketplace_id: ALICE_MARKETPLACE_ID, owner: BOB };
			let event = Event::Marketplace(event);
			System::assert_last_event(event);
		})
	}
...
mod set_marketplace_kind {
	...
	fn set_marketplace_kind() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			Marketplace::set_marketplace_kind(alice, ALICE_MARKETPLACE_ID, MarketplaceType::Private).unwrap();
	...
	fn marketplace_not_found() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			let err = Marketplace::set_marketplace_kind(alice, INVALID_MARKETPLACE_ID, MarketplaceType::Private);
	...
	fn not_the_marketplace_owner() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			let err = Marketplace::set_marketplace_kind(alice, BOB_MARKETPLACE_ID, MarketplaceType::Private);
...
mod set_marketplace_configuration {
	...
	fn set_marketplace_configuration() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			let data_none = MarketplaceData::new(ALICE, MarketplaceType::Public, None, None, None, None);
...
mod list_nft {
	...
	fn list_nft() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			// List NFT.
			Marketplace::list_nft(alice, ALICE_NFT_ID, data.marketplace_id, data.price).unwrap();
	...
	fn list_nft_listing_fee_percentage() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			let sale = Marketplace::listed_nfts(ALICE_NFT_ID).unwrap();
			assert_eq!(sale, data);
			assert_eq!(Balances::free_balance(ALICE), alice_balance - (new_listing_fee * data.price));
	...
	fn nft_already_listed() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			Marketplace::list_nft(alice.clone(), ALICE_NFT_ID, ALICE_MARKETPLACE_ID, 10).unwrap();
	...
	fn account_not_allowed_to_list_not_authorized() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			Marketplace::set_marketplace_kind(alice.clone(), ALICE_MARKETPLACE_ID, MarketplaceType::Private).unwrap();
...
mod unlist_nft {
	...
	fn unlist_nft() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			// List NFT.
			Marketplace::list_nft(alice.clone(), ALICE_NFT_ID, data.marketplace_id, data.price).unwrap();
...
mod buy_nft {
	...
	fn keep_alive() {
		ExtBuilder::new_build(vec![(ALICE, 1000), (BOB, 1000), (CHARLIE, 1000)]).execute_with(|| {
			...
			// List NFT.
			Marketplace::list_nft(alice, ALICE_NFT_ID, CHARLIE_MARKETPLACE_ID, bob_balance).unwrap();
