	...
	fn list_happy() {
		ExtBuilder::default()
			...
			.execute_with(|| {
				...
				let nft_id =
					<NFTs as NFTTrait>::create_nft(ALICE, vec![50], Some(series_id.clone()))
						.unwrap();
...
pub mod set_marketplace_mint_fee {
	pub use super::*;
...
pub mod set_marketplace_logo_uri {
	...
	fn update_logo_uri_happy() {
		ExtBuilder::default().caps(vec![(ALICE, 10000)]).build().execute_with(|| {
			...
			assert_ok!(Marketplace::set_marketplace_logo_uri(
				alice.clone(),
				1,
				updated_uri.unwrap()
			));
...
pub mod set_marketplace_description {
	...
	fn set_description_unhappy() {
		ExtBuilder::default().caps(vec![(ALICE, 10000)]).build().execute_with(|| {
			...
			let nok = Marketplace::set_marketplace_description(
				alice.clone(),
				1,
				raw_too_short_description,
			);
