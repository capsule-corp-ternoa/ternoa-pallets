...
benchmarks! {
	...
	}: _(RawOrigin::Signed(alice.clone()), vec![55], None, 0)
	verify {
	...
	set_nft_royaltie_fee {
		...
		let old_royaltie_fee = NFTs::<T>::data(NFT_ID).unwrap().royaltie_fee;
	...
	verify {
		...
		assert_eq!(NFTs::<T>::data(NFT_ID).unwrap().royaltie_fee, new_royaltie_fee);
	}
}
