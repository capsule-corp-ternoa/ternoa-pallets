...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_account(
			...
			Users::<T>::mutate(&who, |maybe_accounts| {
				if let Some(accounts) = maybe_accounts {
					let pos = accounts.iter().position(|account| account.key == account_key);
					if let Some(pos) = pos {
			...
			let event = Event::UserAccountAdded { user: who, account_key, account_value };
		...
		pub fn add_new_supported_account(
			...
			let event =
				Event::SupportedAccountAdded { key, min_length, max_length, initial_set_fee };
			Self::deposit_event(event);
	...
	impl<T: Config> Default for GenesisConfig<T> {
		fn default() -> Self {
			Self { users: Default::default(), supported_accounts: Default::default() }
		}
	}
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			self.users.clone().into_iter().for_each(|(owner, accounts)| {
