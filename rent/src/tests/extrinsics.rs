...
const FIXED_AUTO_NOREV_NFT_TOKENS_NONE: NFTId = 0;
const FIXED_AUTO_REV_TOK_FLEXTOK_FLEXTOK: NFTId = 1;
const FIXED_MANU_REV_NFT_NONE_NFT: NFTId = 2;
...
fn prepare_tests() {
	...
	Rent::create_contract(
		alice.clone(),
		FIXED_AUTO_NOREV_NFT_TOKENS_NONE,
	...
	assert!(Rent::contracts(FIXED_AUTO_NOREV_NFT_TOKENS_NONE).is_some());
	assert!(Rent::contracts(FIXED_AUTO_REV_TOK_FLEXTOK_FLEXTOK).is_some());
	assert!(Rent::contracts(FIXED_MANU_REV_NFT_NONE_NFT).is_some());
...
mod create_contract {
	...
	fn max_simultaneous_contract_reached() {
		ExtBuilder::new_build(None).execute_with(|| {
			...
			for i in 11..11 + nb_contract_to_create {
...
mod cancel_contract {
	...
	fn cancel_contract() {
		ExtBuilder::new_build(None).execute_with(|| {
			...
			assert!(Rent::queues().available_queue.get(FIXED_AUTO_REV_TOK_FLEXTOK_FLEXTOK).is_none());
			assert!(!nft.state.is_rented);
			// Event check.
...
mod revoke_contract {
	...
	fn revoke_contract_by_renter_fixed() {
		ExtBuilder::new_build(None).execute_with(|| {
			...
			Rent::rent(bob, FIXED_AUTO_NOREV_NFT_TOKENS_NONE).unwrap();

			let alice_balance = Balances::free_balance(ALICE);
			let bob_balance = Balances::free_balance(BOB);

			// Revoke.
			Rent::revoke_contract(alice, FIXED_AUTO_NOREV_NFT_TOKENS_NONE).unwrap();

			// State check.
			let nft = NFT::nfts(FIXED_AUTO_NOREV_NFT_TOKENS_NONE).unwrap();
			assert!(Rent::contracts(FIXED_AUTO_NOREV_NFT_TOKENS_NONE).is_none());
			assert!(Rent::queues().available_queue.get(FIXED_AUTO_NOREV_NFT_TOKENS_NONE).is_none());
			assert!(!nft.state.is_rented);
			assert_eq!(Balances::free_balance(ALICE), alice_balance);
			assert_eq!(Balances::free_balance(BOB), bob_balance + LESS_TOKENS);
			// Event check.
			let event = RentEvent::ContractRevoked {
				nft_id: FIXED_AUTO_NOREV_NFT_TOKENS_NONE,
				revoked_by: ALICE,
			};
			let event = Event::Rent(event);
			System::assert_last_event(event);
