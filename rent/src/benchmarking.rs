...
use frame_support::{assert_ok, assert_noop, traits::Currency, BoundedVec};
...
pub fn prepare_benchmarks<T: Config>() -> () {
	...
	let res = Rent::<T>::create_contract(
		origin::<T>("ALICE").into(),
		NFT_ID_0,
	...
	// assert_noop!(res, Error::<T>::NotEnoughBalanceForCancellationFee);
	assert_ok!(res);
}
